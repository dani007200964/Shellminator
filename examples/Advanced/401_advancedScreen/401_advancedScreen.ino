...
private:
    ...
    float plotData[ 10 ];
    int plotDatSize = sizeof( plotData ) / sizeof( plotData[ 0 ] );

    ShellminatorPlot plot;

    // Create the two ShellminatorPlot object. We will use them side by side in out layout.
    ShellminatorButton buttonTop;
    ShellminatorButton buttonMiddle;
    ShellminatorButton buttonBottom;
    ...
    const char* listDetails[] = {
        ...
        "Configure pin as regular GPIO"
    };    

    ShellminatorListDetailed list;

    Shellminator::shellEvent_t topButtonEvent;
    Shellminator::shellEvent_t middleButtonEvent;
    Shellminator::shellEvent_t bottomButtonEvent;
...
void myLayout::init( Shellminator* parent_p, Stream* channel_p ){
    ...
    plot = ShellminatorPlot( plotData, plotDataSize, "Plot" );
    list = ShellminatorListDetailed( listOptions, listDetails, 3, listText );
